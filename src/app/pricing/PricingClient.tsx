...
export default function PricingClient({ locale = "en", plan, showBanners = false }: PricingClientProps) {
  ...
  const handleSubscribe = async (targetPlan: "pro" | "business") => {
    if (!user) {
  ...
  const renderButton = (targetPlan: "pro" | "business") => {
    ...
    // Current tier matches this plan => show portal button
    if (tier === targetPlan) {
      return (
        <button
        ...
        >
          {subLoading ? t("처리 중...", "Loading...") : t("구독 관리", "Manage Subscription")}
        </button>
      )
    }

    // Already on a higher plan — show disabled state for pro when on business
    ...
    return (
      ...
      >
        {actionLoading === targetPlan
          ...
          : user
            ? targetPlan === "pro"
              ? t("Pro 구독하기 →", "Subscribe to Pro →")
              : t("Business 구독하기 →", "Subscribe to Business →")
      ...
      {showBanners && canceled && (
        <div style={{
          ...
          <p style={{ color: "#ef4444", fontSize: 14, fontWeight: 600 }}>
            {t("결제가 취소되었습니다.", "Payment was canceled.")}
          </p>
        </div>
      )}
